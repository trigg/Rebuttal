...
Join a `voice` room. Triggers a `updateRooms` message for every connected user.

| type           | `leaveroom` | |
|-----------------|-----------|-|
| `roomid`       | string (uuid) | The room the user is leaving |

Leave a `voice` room. Triggers a `updateRooms` message for every connected user.
...
| type           | `removeuser`  | |
|-----------------|-----------|-|
| `touserid`     | string (uuid) or null or undefined | The user intended to be deleted |
| `withvengence` | boolean                            | If true, all messages, uploads etc for the user should be deleted at this point too |

Requires permission `removeUser`. If `touserid` if falsey (null, undefined) then the calling user is deleted. This will trigger an `updateUsers` message to all connected clients
...
Requires permission `removeMessage`. Mask the contents of the message. Servers MUST set the text to `*Message Removed*` and remove `userid`. Clients may decide to not show messages with text `*Message Removed*` and undefined `userid`
...
| `changes`      | list                    | a list of [objects](../object.md) |
